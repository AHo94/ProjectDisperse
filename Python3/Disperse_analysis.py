...
class Disperse_Plotter():
	...
	def Solve(self, filename, ndim=3, Sigma_threshold = False, robustness=False):
		...
		if os.path.isfile(Disperse_data_check_fn):
			...
			Box_boundaries, CP_coordinate_data, Filament_coordinate_data, CP_data, Filament_data = pickle.load(open(Disperse_data_check_fn, 'rb'), encoding='latin')
		else:
		...
		if os.path.isfile(Boundary_check_cachefn):
			...
			BC_instance_variables = pickle.load(open(Boundary_check_cachefn, 'rb'), encoding='latin1')
		else:
		...
		if IncludeSlicing:
			...
			if os.path.isfile(Mask_slice_cachefn):
				...
				Mask_instance_variables, Masked_critpts = pickle.load(open(Mask_slice_cachefn, 'rb'), encoding='latin1')
			else:
...
def Save_NumPartPerFil(name, FilPos, FilID, FilPosNBC, FilIDBC, BoxSize, npart, nsig):
	...
	Common_name_npy = name + '_' + str(npart) + 'part_nsig' + str(nsig) + '_BoxExpand' + str(box_expand) + '.npy'
	#cachefile_distances = cachedir_ppf_distances + name + '_' + str(npart) + 'part_nsig' + str(nsig) + '_BoxExpand' + str(box_expand) + 'TESTING2.p'
	#cachefile_partbox = cachedir_ppf_partbox + name + '_' + str(npart) + 'part_nsig' + str(nsig) + '_BoxExpand' + str(box_expand) + 'TESTING.p'
	#cachefile_ids = cachedir_ppf_ids + name +  '_' + str(npart) + 'part_nsig' + str(nsig) + '_BoxExpand' + str(box_expand) + 'TESTING.p' 
	#cachefile_segIDs = cachedir_ppf_segIDs + name + '_' + str(npart) + 'part_nsig' + str(nsig) + '_BoxExpand' + str(box_expand) + 'TESTING2.p'
	#cachefile_tsols = cachedir_ppf_tsols + name + '_' + str(npart) + 'part_nsig' + str(nsig) + '_BoxExpand' + str(box_expand) + 'TESTING2.p'
	...
	cachefile_distances = cachedir_ppf_distances + Common_name_npy
	#cachefile_partbox = cachedir_ppf_partbox + Common_name_npy
	cachefile_ids = cachedir_ppf_ids + Common_name_npy
	cachefile_segIDs = cachedir_ppf_segIDs + Common_name_npy
	cachefile_tsols = cachedir_ppf_tsols + Common_name_npy
	Particles_read = 0
	def Masking_singleCPU():
		...
		for i in range(len(FilPos)):
			...
			#if i == len(FilPos)-1:
			#	print(idY, idZ)
			#	print(Sent_particles_real)
			#	print(Particle_indices)
			#	print('== ==')
			#	print(filbox)
			#	print(masked_ids_slice)
			#	print(masked_ids)
		...
		#pickle.dump(Masked_ids, open(cachefile_ids, 'wb'), protocol=2)
		#pickle.dump(Part_box, open(cachefile_partbox, 'wb'), protocol=2)
		print(Masked_ids)
		print(Masked_ids_nonmerge)
		np.save(cachefile_ids, Masked_ids)
		#np.save(cachefile_partbox, Part_box)
	...
	Nfils_iterate = list(range(len(FilPos)))
	if os.path.isfile(cachefile_ids):
		...
		#Part_box = pickle.load(open(cachefile_partbox, 'rb'), encoding='latin1')
		Masked_ids = np.load(cachefile_ids)
		#Part_box = np.load(cachefile_partbox)
		print('done')
	else:
		...
		if name == 'symm_C':
			ParticlePos = ReadGadgetFile.read_file_ascii('/mn/stornext/d5/aleh/SymmC_data/SymmC_particles.solve')
		else:
			ParticlePos = ReadGadgetFile.read_file(name)
		Particles_read = 1
		#Masked_ids, Part_box = Masking_singleCPU()
		Masked_ids = Masking_singleCPU()
	#print(len(FilPos))
	#print(len(FilPosNBC))
	#print(Masked_ids)
	#sys.exit(1)
	def ZMQ_call():
		...
		subprocess.call("./SpawnWorkers_python3.sh 40 0", shell=True)
		subprocess.call("./RemoteConnect_python3.sh", shell=True)
		time.sleep(5)
		time_dist = time.time()
		# Sends data
		...
		for k in range(Fil_slices+1):
			...
			Masked_ids_sent = Masked_ids[k*Numfil_p_slice:(k+1)*Numfil_p_slice]
			Sent_Pboxes = np.array([ParticlePos[Masked_ids_sent[i]] for i in range(len(Masked_ids_sent))])
			...
			if len(Sent_filaments):
				for i in range(len(Sent_filaments)):
					...
					ZMQAS.send_zipped_pickle(sender, [Sent_filaments[i], Sent_Pboxes[i], i], protocol=2)
				...
				for j in range(len(Sent_filaments)):
					...
					if socks.get(data_receive) == zmq.POLLIN:
						data = ZMQAS.recv_zipped_pickle_p3(data_receive)
				...
				ID_ordering = np.concatenate((ID_ordering, np.asarray(ID_ordering_temp)+k*Numfil_p_slice))
		...
		context.term()
		#pickle.dump(Distances[Sorted], open(cachefile_distances, 'wb'), protcol=2)
		#pickle.dump(FilamentAxis[Sorted], open(cachefile_segIDs, 'wb'), protcol=2)
		#pickle.dump(T_solution[Sorted], open(cachefile_tsols, 'wb'), protocol=2)   # Only include if analytical method is used
		np.save(cachefile_distances, Distances[Sorted])
		np.save(cachefile_segIDs, FilamentAxis[Sorted])
		np.save(cachefile_tsols, T_solution[Sorted])
		return Distances, FilamentAxis
	def ZMQ_call_alldata():
		print(FilPosNBC[0])
		context = zmq.Context()
		context.linger = 0
		# Socket to send messages on
		...
		for i in range(NumFils):
			...
			if i == NumFils-1:
				print("all data sent")
			if i == NumFils//2:
				print("Halfway through data sending")
		# Give time to send data
		time.sleep(5)
		#print 'Looping through data receiving'
		...
		control_sender.close()
		context.term()
		pickle.dump(Distances[Sorted], open(cachefile_distances, 'wb'), protocol=2)
		pickle.dump(FilamentAxis[Sorted], open(cachefile_segIDs, 'wb'), protocol=2)
		pickle.dump(T_solution[Sorted], open(cachefile_tsols, 'wb'), protocol=2)   # Only include if analytical method is used
		return Distances, FilamentAxis
	...
	if os.path.isfile(cachefile_distances):
		...
		#Distances, FilamentAxis = ZMQ_call_alldata()
	print('check check')
