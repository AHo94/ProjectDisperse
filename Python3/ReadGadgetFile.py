...
class Read_Gadget_file():
	...
	def read_file(self, model):
		...
		"""
		print('Reading gadget files for model ' +model+ '...')
		numfiles = 512
		nparticles = 512
...
def read_file(model):
	...
	PartPos = np.empty((nparticles**3,3),np.float32)
	PartVel = np.empty((nparticles**3,3),np.float32)
	PartIDs = np.empty((nparticles**3),np.int32)
	istart = 0
	for i in np.arange(0, numfiles):
		file=filename+str(i)
		f=open(file, 'rb')
		...
		PartPos[istart:(istart+npart),:] = thispos
		PartVel[istart:(istart+npart),:] = thisvel
		PartIDs[istart:(istart+npart)] = thisID
		istart = istart + npart

	print('finished reading particles, ')
	# Note: Velocity units in km/s and position in kpc/h
	# Converting units from kpc/h to Mpc/h
	PartPos = PartPos/1000.0
	File_read = 1
	return PartPos

def read_file_ascii(filedir):
